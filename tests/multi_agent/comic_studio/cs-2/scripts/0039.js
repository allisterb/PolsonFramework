...
const REGIONS = [
    ...
    ['boot',            370, 815, 545, 1000], ['belt+buckle',     490, 470, 690, 555]
];
...
table(REGIONS.map(R => {
    ...
    return { region: R[0], inker: a.toFixed(1), final: b.toFixed(1), delta: (b - a).toFixed(1),
...
function overall(img) {
    ...
    return 'coverageErr ' + (100 * bad / (same + bad)).toFixed(2) + '%   meanRGBdist ' + (s / n).toFixed(1);
}
log('stage3 inker : ' + overall(s3));
log('stage4 final : ' + overall(s4));
exit('final scoring complete');
