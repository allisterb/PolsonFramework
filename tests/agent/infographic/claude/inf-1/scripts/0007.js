...
for (let y = 22; y < H; y += 44) {
    ...
    for (let h = 0; h < holes.length; h++) {
        ctx.fillStyle = PAPER;
        ctx.beginPath(); ctx.arc(holes[h], y, 11, 0, Math.PI * 2); ctx.fill();
...
const TEAR_Y = 1596, FOOTER_TOP = TEAR_Y + 22;
...
ctx.fillText('SEVEN MILESTONES / 53 YEARS / DRAWN TO SCALE', sheet.x + 14, 162);
