Stage.begin('Critic');
Stage.note('Refinement pass 2, driven by the regional comparison. (a) Shader kept for the CAPE only, where it cut error 37.7->22.7 and 30.0->23.0, and a fifth plane added for the mid-right region that had no centre near it and blew out to 44.8; dark (632,617) #ac382c, lit (713,813) #fb4434. (b) Suit REVERTED to linear gradients — the shader made torso 24.6->30.4 and legs 18.3->25.9, so it loses there and I am not keeping a change that only helps the average. (c) Belt was the worst non-cape region in both stages and nobody had looked: it is not a linear ramp, it is dark at BOTH ends (#D02E2E left, #D33030 right) and #F44336 across the middle, a cylindrical highlight — and its base red was brighter than I had it. (d) Buckle was 8px left of true; measured span is x575-615 at y524. (e) Face top edge now follows the traced fringe point-for-point instead of my smoothed approximation.');

const canvas = createCanvas(1024, 1024);
const ctx = canvas.getContext('2d');
const H = h => [parseInt(h.substr(1, 2), 16) / 255, parseInt(h.substr(3, 2), 16) / 255, parseInt(h.substr(5, 2), 16) / 255];

// Partition-of-unity blend of locally measured linear colour planes.
...
const capeShader = planeBlend([
    { d: [488, 414], cd: H('#8f1c07'), l: [312, 356], cl: H('#e74a3e') },   // upper-left lobe
    { d: [441, 558], cd: H('#b6382c'), l: [189, 512], cl: H('#ff4536') },   // left wing
    { d: [517, 637], cd: H('#a03023'), l: [303, 823], cl: H('#fd4231') },   // lower / tail
    { d: [643, 416], cd: H('#662820'), l: [822, 564], cl: H('#dc2e15') },   // far lobe behind body
    { d: [632, 617], cd: H('#ac382c'), l: [713, 813], cl: H('#fb4434') }    // mid-right, was uncovered
]);
...
const torsoGrad = lin(0, 300, 0, 510, PAL.suitLit, PAL.suitDeep);
fillShape([
...
// Belt: horizontal, dark at both ends, #F44336 across the middle.
...
fillShape([[492,484],[506,486],[550,492],[600,502],[650,514],[678,520],[674,552],[640,550],
           [600,548],[556,538],[512,516],[494,502]], beltGrad);
fillShape([[592,500],[604,510],[613,524],[612,538],[600,549],[590,552],[578,542],[571,528],
           [572,512],[581,504]], PAL.gold);
...
// Face: top edge follows the traced hair/skin boundary point-for-point.
