// ============================================================================
// cs-1 · Stage 2 · CRITIC — refinement pass 2
// v1 defects: value range compressed to 10-25% grey with no true dark; the far
// ear carried a concentric ellipse that read as a second contour; no ear
// interiors anywhere; the right rabbit had no muzzle division; whiskers still
// too few, too long, too even.
// ============================================================================
Stage.begin('Critic');
Stage.note('Refinement pass 2. v1 held up structurally and failed on value: the whole image sat between 10 and 25 percent grey, so the trunk — the darkest mass in the reference — carried the same weight as fur. Deepening the trunk to near-black across four hatch directions, taking the right rabbit to a solid mid-dark with a core shadow down its viewer-right, and giving every ear an interior. The far ears concentric inner ellipse is deleted outright: it read as a second outline and made the ear look like a plate.');
...
const A = {
    L: {
        ...
        whiskerRoot: [231, 262], whiskerEnds: [[152, 238], [146, 254], [148, 270], [158, 286], [172, 298]]
    },
    R: {
...
const SH = {
    trunkCore: [[110, 0], [176, 0], [180, 190], [118, 190]],
...
// --- trunk: four directions to a true near-black ----------------------------
knock(REG.trunk);
...
// --- stems, leaves rooted on the stem ---------------------------------------
...
// --- LEFT rabbit: the light animal, but with real interiors -----------------
knock(REG.lFarEar);
...
strokeS([[24, 118], [46, 152], [44, 196]]);        // far-ear inner edge — an edge, not a second outline
set(G.graphite, 0.95, 0.75); strokeS([[86, 168], [98, 180], [92, 192]]);

// --- RIGHT rabbit: the dark animal ------------------------------------------
knock(REG.rEarFar);
hatch(REG.rEarFar, null, 82, 2.8, 0.65, 0.30, G.soft);
hatch(REG.rEarFar, SH.rEarFarIn, 82, 2.0, 0.7, 0.26, G.graphite);
knock(REG.rEarNear);
hatch(REG.rEarNear, null, 82, 2.8, 0.65, 0.28, G.soft);
hatch(REG.rEarNear, SH.rEarNearIn, 78, 2.0, 0.7, 0.28, G.graphite);
knock(REG.rBody);
hatch(REG.rBody, null, 72, 2.3, 0.7, 0.34, G.graphite);
hatch(REG.rBody, null, 16, 4.6, 0.6, 0.18, G.soft);
hatch(REG.rBody, SH.rBodyCore, 72, 1.9, 0.75, 0.30, G.black);
hatch(REG.rBody, SH.rNeck, 108, 2.6, 0.65, 0.20, G.dark);
...
set(G.graphite, 1.25, 1); strokeS(ell(A.R.lidE.c[0], A.R.lidE.c[1], A.R.lidE.rx, A.R.lidE.ry, A.R.lidE.rot, 32), true);
...
whiskers(A.L.whiskerRoot, A.L.whiskerEnds, 0.42, 0.55);
