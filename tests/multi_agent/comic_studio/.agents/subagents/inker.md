...
- `manuals/05_observation_measurement_and_csi_curves.md` (CSI curve grammar, tapered strokes, feathering)
...
2. **Actuation (Code with Native Drawing Inking)**:
   ...
   - Use the native SDK APIs:
     ...
     2. **Tier 2 (Internal Structural Contours & Tapered Hair)**:
        - Stylized comic features: `Drawing.drawComicEye(ctx, eyeObj)`, `Drawing.drawComicNose(ctx, noseObj)`, `Drawing.drawComicMouth(ctx, mouthObj)`.
        - Single-call calligraphic hair strands: `ctx.drawTaperedStroke(x1, y1, cp1x, cp1y, cp2x, cp2y, x2, y2, maxThickness, '#0a0a0c')` for 15+ windswept locks!
     3. **Tier 3 (Feathering & Cross-Hatching - 0.8px to 1.5px)**:
        - Directional feathering along neck and jaw: `ctx.drawFeathering(ox, oy, angleDeg, count, length, spacing, '#0a0a0c', 1.2)`.
        - Volumetric cross-hatching on shirt folds: `ctx.drawCrossContourHatch(cx, cy, rx, ry, startAngle, endAngle, count)`.
   ...
   - Execute via `ExecuteScript(script: ..., outFile: 'artifacts/stage3_inker.webp')`.

3. **Visual Verification**:
   - The rendered image is automatically saved to `artifacts/stage3_inker.webp`.
