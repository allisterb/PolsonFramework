Stage.begin('Inker');
Stage.note('Inking as tone, not stroke — the deliberate departure from my brief, recorded so it reads as a decision rather than an omission. Three-tier hierarchy mapped onto the references own vocabulary: PRIMARY = the deep occlusion seams that carry contour (the wedge under the bent arm, measured #5B2923 at its core, running (502,402) to (412,520) and separating the arms blue from the torsos); SECONDARY = the boundary tones between adjacent reds (cape/torso #772519, gauntlet cuff #A52714, boot/cape #B32E1D); TERTIARY = none, because the reference has none. Contours corrected from the delta report: the bent arms upper arm is a SEPARATE blue strip (448-472 at y440, 424-448 at y472) that I had merged into the torso; the chest shows through the collar notch at 488-520 by y296; the hair crown is a narrow peak at (609,62) with a notch in the upper-left lock, not the broad dome I painted.');
...
const PAL = {
    ...
    seamDeep: '#5b2923', seamMid: '#772519',
    collarLit: '#a8382d', collarDeep: '#74261b',
...
// === 2. TORSO — top bumps up to y=288 at x488-520 so the chest shows in the
//        collar notch, as the reference does.
const TORSO = [
    ...
    [672,492],[668,504],[620,502],[580,496],[556,490],[528,482],[500,478],[460,472],[448,462],
    [449,440],[450,434],[448,424],[442,410],[434,400],[430,390],[434,378],[440,366],[444,354],
    [450,342],[456,326],[466,312]
];
fillShape(TORSO, lin(0, 300, 0, 510, PAL.suitLit, PAL.suitDeep));

// === 3. LEGS — top pulled in; the belt covers everything above y~545 anyway.
const LEGS = [
...
fillShape(LEGS, lin(634, 540, 520, 812, PAL.suitLit, PAL.suitLeg));

// === 4. PRIMARY SEAM — the occlusion wedge under the bent arm. Tight, traced
//        from the reference's dark band, so blue survives on BOTH sides of it.
ctx.save();
ctx.filter = Skia.ImageFilter.blur(4, 4);
fillShape([[504,400],[496,414],[496,424],[488,432],[480,440],[480,448],[472,456],[464,464],
           [456,472],[440,480],[432,488],[432,496],[424,504],[416,512],[414,520],[430,520],
           [444,512],[452,504],[462,496],[476,488],[500,480],[510,472],[512,464],[512,448],
           [512,432],[512,420],[510,408]], PAL.seamDeep);
ctx.restore();

// === 5. BENT ARM — blue strip, forearm, fist ================================
fillShape([[456,392],[444,406],[450,420],[452,434],[448,450],[440,462],[428,472],[430,480],
           [444,478],[458,470],[468,458],[476,442],[482,424],[484,408],[478,396]],
          lin(430, 400, 500, 470, PAL.armDeep, PAL.suitDeep));
ctx.save();
ctx.globalAlpha = 0.92;
fillShape([[354,350],[392,342],[440,408],[470,452],[452,476],[418,448],[380,398]], '#c93a2b');
ctx.restore();
...
// === 8. COLLAR — darkened to the measured #74261b at its lower-left edge ====
fillShape([[424,308],[436,284],[456,266],[484,252],[524,242],[576,238],[628,238],[672,240],
          ...
          lin(440, 308, 640, 246, PAL.collarDeep, PAL.collarLit));

// === 9. HEAD ===============================================================
// Hair: narrow crown peak at (609,62), notch in the upper-left lock, and the
// two locks at y236-248 that the smoothed outline had erased.
...
// Secondary seam where the boot overlaps the cape — measured #B32E1D.
ctx.save();
ctx.globalAlpha = 0.5;
ctx.filter = Skia.ImageFilter.blur(5, 5);
fillShape([[452,812],[462,824],[452,846],[440,872],[430,890],[420,884],[432,856],[442,830]], '#8f2418');
ctx.restore();
