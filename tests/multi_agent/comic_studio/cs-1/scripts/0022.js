// cs-1 — "Two rabbits", consolidated master (identical to artwork.js).
Stage.begin('Critic');
Stage.note('Final render. Three changes over refined_v2: the right rabbit gets a fourth hatch direction and higher alphas so it sits clearly darker than the left, as the reference has it; the two right-hand stems are shortened and the third is detached so the group stops reading as a closed triangle; and the trunk region runs 16px lower with a soft foot pass so it does not end on a hard horizontal cut.');

const RW = 620, RH = 336, S = 2;
const G = { ground: '#faf8f2', black: '#141414', dark: '#242424', graphite: '#3d3d3d', soft: '#5f5f5f', pale: '#8a8a8a' };
const A = {
    L: {
...
function drawTrunk() {
    ...
    hatch(REG.trunk, SH.trunkFoot, 70, 3.4, 0.8, 0.16, G.soft);
...
function drawLeftRabbit() {
    ...
    strokeS(A.L.nearEarRidge); strokeS([[24, 118], [46, 152], [44, 196]]);
    set(G.graphite, 0.95, 0.75); strokeS([[86, 168], [98, 180], [92, 192]]);
}
...
drawTrunk(); drawStems(); drawLeftRabbit(); drawRightRabbit(); grass(0, 60, 0.85); drawFeatures();
canvas;