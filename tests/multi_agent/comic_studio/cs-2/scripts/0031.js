Stage.begin('Critic');
Stage.note('Scoring refinement pass 1 against stage 3, on the same two metrics, so the change is proven rather than assumed.');
...
function score(file) {
    ...
    return { file: file.replace('artifacts/', ''), matched: same, wrong: bad,
