@echo off
rem Installs the ADK agent's Python dependencies into the repo's SEPARATE python-adk venv.
rem
rem This mirrors src\webapp\install.cmd exactly but for a DIFFERENT environment. The two runtimes are
rem kept apart on purpose: ADK caps websockets<16 where google-antigravity has no upper bound, so a
rem shared venv would let one install silently win and leave neither lock describing what is there.
rem The Antigravity runtime uses the original python\ venv; ADK uses python-adk\. Pick one per run.
rem
...
set "VENV_PIP=%REPO_ROOT%\python-adk\Scripts\pip.exe"
...
if not exist "%VENV_PIP%" (
    ...
    echo        create it with:  python -m venv "%REPO_ROOT%\python-adk" 1>&2
