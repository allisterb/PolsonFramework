// ============================================================================
// cs-1 · Stage 1 · PENCILER — construction sheet, pass 4 (stage final)
// Overlay pass 3 on the reference: everything tracks except the far ear, which
// was 13px narrow on its right (drawn edge x97, measured edge x110 at y134).
// ============================================================================
Stage.begin('Penciler');
Stage.note('Pass 4. The multiply overlay of pass 3 put both eye clusters, both crania, the right chest edge, the crown and jaw chains and the trunk edges on the reference. One mass was still wrong: the far ear at rx46 reached x97 where the reference reaches x110, so it is widened to rx52 c(58,134) — chord 109.5 against a measured 110. Right-ear positions LOOK 5px left in the overlay but the row scans give 397-445 and 459-525 at y=0 across three rows, so I am trusting the measurement over my eye and recording the discrepancy rather than nudging to taste.');
...
const ANCHORS = {
    ...
    L: {
        farEar: { c: [58, 134], rx: 52, ry: 67, rot: -24 },   // FITTED: chord 109.5 vs measured 110
        nearEar: {
        ...
        cranium: { c: [172, 232], r: 62 },
        muzzle: { c: [248, 262], r: 34 },
        noseTip: [281, 258],
    ...
    R: {
        ...
        cranium: { c: [430, 132], r: 70 },
        muzzle: { c: [398, 188], r: 44 },
...
function drawBackground() {
    ...
    smooth([[80, 0], [80, 60], [82, 100], [86, 130], [88, 140]]);
...
function drawLeft() {
    const L = ANCHORS.L, fe = L.farEar;
    set(PENCIL.graphite, 1.6, 1);
    smooth(ellipsePts(fe.c[0], fe.c[1], fe.rx, fe.ry, fe.rot, 40), true);
    set(PENCIL.graphiteSoft, 0.8, 0.85);
    smooth(ellipsePts(fe.c[0] + 3, fe.c[1] + 4, fe.rx - 17, fe.ry - 18, fe.rot, 32), true);
    set(PENCIL.graphite, 1.6, 1);
...
function drawLabels() {
    ...
    ['L.farEar rx52 ry67 -24deg', 6, 58], ['L.nearEar tip(136,45)', 92, 38],
