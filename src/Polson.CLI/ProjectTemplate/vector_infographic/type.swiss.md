...
> [!IMPORTANT]
> **Vector substitution.** SVG has no text flow, so `ctx.measureWrappedText` is unavailable.
> Measure each candidate line with `element.getBBox()` — real font metrics, agreeing with the
> canvas measurement — and place the lines yourself as `<tspan>`s or separate `<text>` nodes.
> The grid discipline this variant is built on is unaffected; only the wrapping is manual.
