#!/usr/bin/env bash
# Installs the studio's Python dependencies into the repo's virtual environment.
#
...
# works whether it is run as ./install.sh from src/studio or as src/studio/install.sh from the repo root.
...
# Which lock. The default serves the studio and reads runs, and needs no agent SDK — `studio/` has
# no reference to one and `runs._drive` imports the driver lazily. Pass `driver` to also get the
# Antigravity SDK, which is what the page's Start button and `python -m orchestrator` need. The
# difference is 17 packages against 52, which is the whole reason the two lists are separate.
#
#   ./install.sh            observe a run driven from your own agent host
#   ./install.sh driver     start runs from the page, or from the terminal
if [[ "${1:-}" == "driver" ]]; then
    requirements="${script_dir}/requirements-driver.txt"
else
