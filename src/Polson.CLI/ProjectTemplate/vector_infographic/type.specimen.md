...
> [!IMPORTANT]
> **Vector substitution.** The aged-paper ground still works: requisition it with
> `Assets.material('aged laid paper…')` and inline it with `paper.image(material, 0, 0, w, h)`,
> which embeds it as a data URI. What is not available is a procedural grain shader — the paper
> texture has to be a real image rather than generated noise.
