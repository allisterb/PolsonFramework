...
Stage.note('User requested: Make the background green. Adapting the design language to a high-contrast Neo-Brutalist green aesthetic (vibrant green ground #D8F3DC / retro green-bar tint with deep forest green tractor margins #1B4332 and sharp platen ink #111111), maintaining full accessibility and contrast across all elements.');

Stage.begin('Detail');
Stage.note('Re-rendering detailed infographic with the requested green background, updated tractor margins, and neo-brutalist green styling.');
...
// Continuous Fanfold Fold / Perforation across the green page
const foldY = 1040;
ctx.strokeStyle = '#5B9F7D';
ctx.lineWidth = 2;
ctx.setLineDash([8, 8]);
ctx.beginPath();
...
ctx.fillStyle = '#246143';
ctx.font = '900 11px Consolas, monospace';
ctx.fillText('>>> FOLD PERFORATION // GREEN-BAR CONTINUOUS FEED · PAGE 01 / 02 >>>', marginW + 12, foldY - 6);
...
ctx.fillText('SYSTEM://NET_HISTORY.LOG · CONTINUOUS GREEN-BAR FEED · 53 YEARS', headX + 16, headY + 24);
