// =============================================================================
// artwork.js — cs-2, comic_studio
//
// Reproduction of reference_images/comic1.png as executable graphics code.
//
// Every coordinate and every colour in this file was MEASURED off the reference
// by sampling it in the sandbox (Skia.Image.load + getPixel), not estimated by
// eye. The measurement passes are in scripts/ in execution order; the reasoning
// is in critique_log.md.
//
// The reference is a flat vector illustration (Noto-style emoji, Material
// Design palette) on a TRANSPARENT ground. It has no line art at all, so this
// file has no strokes: the drawing is entirely silhouette plus gradient, which
// is how the original is built.
//
// Layer order is back-to-front and matters. In particular the cape is behind
// the torso but IN FRONT of the trailing leg, which is why that leg simply ends
// at y=690 with no visible foot.
// =============================================================================

const WIDTH = 1024, HEIGHT = 1024;

// -----------------------------------------------------------------------------
// ANCHORS — the Penciler's measurements. Named once, here, so a later pass can
// refer to a landmark instead of re-deriving a number.
// -----------------------------------------------------------------------------
const ANCHORS = {
    head:   { cx: 600, crownY: 62, chinY: 272, hairL: 498, hairR: 696,
              hairlineY: 126, faceWidestY: 210, faceL: 514, faceR: 682,
              neckTip: { x: 601, y: 308 } },
    eyeL:   { cx: 561, cy: 195, r: 10 },      // measured bbox 551-571 x 185-206
    eyeR:   { cx: 635, cy: 193, r: 10 },      // measured bbox 625-645 x 183-204
    browL:  { cx: 560, cy: 172, w: 32 },
    browR:  { cx: 635, cy: 171, w: 30 },
    nose:   { cx: 597, cy: 220, w: 18 },      // darker amber #e59600, x588-606 y214-226
...
// -----------------------------------------------------------------------------
// PALETTE — recovered by colour census over the reference. These are Material
// Design values, which is what the original emoji set uses.
// -----------------------------------------------------------------------------
const PALETTE = {
    // cape gradient endpoints, one pair per gradient region of the original
...
// -----------------------------------------------------------------------------
// Traced contours. Each is the reference's own silhouette, sampled per scanline.
// -----------------------------------------------------------------------------
const SHAPES = {
    cape: [
        ...
        // bottom edge, including the notch between the near leg and the far lobe
    ...
    // Torso. Below y=400 the left edge is the RIGHT wall of the occlusion gap;
    // the cape's dark core shows through that gap and does the contour work.
    torso: [
    ...
    // The bent arm's upper arm — a separate blue strip, LEFT wall of the same gap.
    bentUpperArm: [
    ...
    // Gauntlet: fist at the top, wrist notch on the left at y~72, forearm to the elbow cuff.
    gauntlet: [
    ...
    // Collar is notched: the chest shows through from x488 by y=296.
    collar: [
    ...
    // Face. The top edge is the traced hair/skin fringe boundary, point for point.
    face: [
...
// Helpers
// -----------------------------------------------------------------------------
const rgb01 = h => [parseInt(h.substr(1, 2), 16) / 255,
...
// so a per-scanline trace reads as a drawn curve rather than a staircase.
...
// The cape is not one gradient. In the original it is several paths, each with
// its own linear gradient, so no single surface fits it: a radial fit left 24
// units of mean error and a global quadratic did no better. This blends five
// locally-measured linear planes with inverse-FOURTH-power distance weights —
// a partition of unity. Inverse-square was tried first and bled the planes into
// each other, costing 4 units in the lower/tail region; the sharper falloff
// keeps each plane governing its own neighbourhood.
function makePlaneBlendShader(regions) {
    ...
    return Skia.Shader.sksl(
        decl + `half4 main(float2 coord) {\n${body}\n float4 o = float4(acc / wsum, 1.0); return half4(o); }`,
        uniforms);
}
function capeShader() {
    const P = PALETTE;
...
// -----------------------------------------------------------------------------
// Layers, back to front. One function per layer.
// -----------------------------------------------------------------------------
function drawCape(ctx)  { fillShape(ctx, SHAPES.cape, capeShader()); }        // Colorist

function drawTorso(ctx) {                                                     // Colorist
    fillShape(ctx, SHAPES.torso, lin(ctx, 0, 300, 0, 510, PALETTE.suitLit, PALETTE.suitDeep));
}

function drawBentArm(ctx) {                                                   // Colorist / Inker
    fillShape(ctx, SHAPES.bentUpperArm, lin(ctx, 430, 396, 500, 470, PALETTE.armDeep, PALETTE.suitDeep));
    ctx.save();
    ctx.globalAlpha = 0.92;                       // forearm sits over the cape, low contrast
...
function drawLegs(ctx) {                                                      // Colorist
    fillShape(ctx, SHAPES.legs, lin(ctx, 634, 540, 520, 812, PALETTE.suitLit, PALETTE.suitLeg));
}

function drawBelt(ctx) {                                                      // Colorist
    // Dark at BOTH ends, #f44336 across the middle — a cylindrical highlight,
    // not a linear ramp. Reading it as a ramp cost 4 units of error.
...
function drawEmblem(ctx) {                                                    // Colorist
    fillShape(ctx, SHAPES.emblem, PALETTE.gold);
    // Inner mark is a straight-sided triangle; checked against y=360/368/384/392.
    ctx.beginPath();
...
function drawRaisedArm(ctx) {                                                 // Colorist
...
function drawCollar(ctx) {                                                    // Colorist
...
function drawHead(ctx) {                                                      // Colorist
...
function drawFeatures(ctx) {                                                  // Detailer
    ...
    // Smile: corners at (574,236) and (622,236), base dipping to (598,253).
...
function drawBoot(ctx) {                                                      // Colorist
    fillShape(ctx, SHAPES.boot, lin(ctx, 555, 895, 360, 920, PALETTE.bootDeep, PALETTE.bootLit));
}

// -----------------------------------------------------------------------------
// Compose. Ground stays transparent, as the reference's is.
// -----------------------------------------------------------------------------
