...
- ~~**No `<title>` or `<desc>`.**~~ **Fixed 2026-09-07 — and now the deliverable should carry both.** `paper.title(text)` sets the accessible name and `paper.desc(text)` the long description; any element takes its own, so a bar can name the quantity it stands for. The title is **inserted first and replaces any existing one**, because the accessible name comes from the first `<title>` child — one appended after the artwork names nothing. An SVG with no title is an unlabelled image however carefully it is drawn, so give every deliverable one.
