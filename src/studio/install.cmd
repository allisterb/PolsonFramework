@echo off
rem Installs the studio's Python dependencies into the repo's virtual environment.
rem
...
rem Which lock. The default serves the studio and reads runs, and needs no agent SDK - `studio/` has
rem no reference to one and `runs._drive` imports the driver lazily. Pass `driver` to also get the
rem Antigravity SDK, which is what the page's Start button and `python -m orchestrator` need. The
rem difference is 17 packages against 52, which is the whole reason the two lists are separate.
rem
rem   install.cmd            observe a run driven from your own agent host
rem   install.cmd driver     start runs from the page, or from the terminal
set "REQUIREMENTS=%SCRIPT_DIR%requirements.txt"
if /i "%~1"=="driver" set "REQUIREMENTS=%SCRIPT_DIR%requirements-driver.txt"
