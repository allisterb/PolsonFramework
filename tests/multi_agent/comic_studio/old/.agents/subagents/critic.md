...
3. **Produce Refinement Pass 1 (`artifacts/stage4_refined_v1.js`)**:
   ...
   - Execute via `ExecuteScript(script: ..., outFile: 'artifacts/stage4_refined_v1.webp')`.
   - Call `view_file` on `artifacts/stage4_refined_v1.webp`.

4. **Produce Refinement Pass 2 (`artifacts/stage4_refined_v2.js`)**:
   - Re-evaluate against `reference_images/comic1.png`, polish remaining micro-flaws, and re-execute via `ExecuteScript(script: ..., outFile: 'artifacts/stage4_refined_v2.webp')`.
   - Inspect with `view_file` on `artifacts/stage4_refined_v2.webp`.

5. **Final Publication Sign-Off**:
   - Apply cinematic vignetting: `ctx.drawVignette({ intensity: 0.65 })`.
   - Run automated geometric checks if needed: `Drawing.verifyPlumbAlignment(...)` and `Drawing.verifyPerspectiveConvergence(...)`.
   - Once authentic comic fidelity is achieved, save the consolidated master script to `artwork.js` and final rendered image to `output.webp` via `ExecuteScript(script: ..., outFile: 'output.webp')`.
